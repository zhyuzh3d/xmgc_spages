...
if (host.indexOf('test') > -1) {
    ...
    gem.host = '/';
    gem.filesite = 'http://120.55.90.62:8000';
    gem.apiprefix = "/project/index.php/api/";
    gem.cdnprefix = "/res/";
    //baidu统计
    var _hmt = _hmt || [];
    (function () {
        var hm = document.createElement("script");
        hm.src = "//hm.baidu.com/hm.js?0f4784a46000772ae63f6df4c3d2ff3c";
        var s = document.getElementsByTagName("script")[0];
        s.parentNode.insertBefore(hm, s);
...
gem.fileTypes = {
    ...
    xls: '.xlsx,.xls',
    xlsx: '.xlsx',
    picx: '.png,.jpg,.gif,.jpeg,.mp4,.webm,.mkv,.m4v',
...
gem.initApp = function (app) {
    ...
    //初始化$rootScope变量和函数
    app.run(function ($rootScope, $location, $anchorScroll, $sce) {
        //设置提示位置
        gem.setToastr();
        ...
        $rootScope.isVideoUrl = function (url) {
            var posn = url.lastIndexOf('.');
            if (posn == -1) return false;

            var ext = url.substr(posn);
            var vstr = ['.mp4', '.webm', '.mkv', '.mov','.m4v'];
            if (vstr.indexOf(ext.toLowerCase()) != -1) return true;
            return false;
        };
        ...
        $rootScope.hidePop = function () {
            ...
            console.log($rootScope.hidePopFn);
            $rootScope.bodyScroll = 'auto';
        };
        ...
        $rootScope.showPop = function (tmpurl, opt, fn) {
            if (fn && typeof (fn) == 'function') {
                console.log(fn, 'ffffffn');
                $rootScope.hidePopFn = fn;
            };
            console.log(fn, 'ffnfnfnfnfn');
        ...
        function initQN(type, event, resfn, qnbox, container, choosebtn, progressid, choosefiles, cb, dt) {
            ...
            var uploader1 = Qiniu.uploader({
                ...
                init: {
                    ...
                    'FilesAdded': function (up, files) {
                        console.log('i am adding files!', up.id, $(event.target), files);
                    ...
                    'FileUploaded': function (up, file, info) {
                        ...
                        //七牛百分号转义
                        fileurl = sourceLink.replace(/%/g, '%25');
            ...
            // plupload.addEvent(document.getElementById(choosebtn), 'click', function(e) {
            //     // var input = document.getElementById(uploader1.id + '_html5');
            //     // if (input && !input.disabled) {
            //     //     input.click();
            //     // } // if
            //     $('#'+container+' div.moxie-shim input[type=file]').focus().trigger('click');
            //
            //     console.log('choose btn  - - -- - -- - --is  clickcekekekekekeke', uploader1,uploader1.id);
            //     e.preventDefault();
            // });
            ...
            setTimeout(function () {
                ...
                switch (type) {
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        filetypes = gem.fileTypes.xls;
                        break;
                    case 6:
                        filetypes = gem.fileTypes.xlsx;
                        break;
                    case 7:
                        filetypes = gem.fileTypes.picx;
                        break;
                    default:
                        filetypes = gem.fileTypes.all;
                        break;
        ...
        $rootScope.uploadFile = function (type, event, resfn, cb, dt) {
            ...
            console.log($("." + btn_id), '>>>>>>>>>>>>>>>>', $("." + btn_id).length);
            ...
            console.log(qnbox, '88888888888888888888888');
            //console.log($('.progress'),"7777777777777777777777777");
